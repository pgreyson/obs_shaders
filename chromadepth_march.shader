...
// Baked field: a = 0.5 + d/2 (see bake header — alpha ≥ 0.5 passes the
// chain unmodified; near-zero alpha arrives black). d = 2(a − 0.5).
float field_exact(float x, float y) {
    return saturate(2.0 * (image.Sample(textureSampler,
                                        safe_uv(x, y)).a - 0.5));
}

// Source color — straight passthrough (probes 3/4 confirmed: rgb arrives
// unmodified when alpha ≥ 0.5; no division needed).
float3 src_color(float x, float y) {
    return image.Sample(textureSampler, safe_uv(x, y)).rgb;
}
...
float depth_field(float x, float y, float f0) {
    return min(field_exact(x, y), 0.995) * edge_taper(x, f0);
}
...
float splat_hits(float sc, float du, float p, float dir, float f0,
    ...
    return hits;   // caller applies the >=2 rule / >=1 fallback
}
...
// >=2 subsamples = primary acceptance (kills straggler-ghost bins at run
// rims). >=1 fallback accepts ONLY inside a CONTINUOUS depth field (both
// pixel neighbors within 0.1 of the candidate): on a smooth gradient every
// dest bin has exactly ONE legitimate claimant, and 8-bit baked depth can
// split its 4 subsamples 1/3 across a bin line — without the fallback the
// >=2 rule paints black stripes across every gradient (16-22k strays on
// the ramp scene = the live-synth "severe distortion"). Rim stragglers
// have a backdrop neighbor and still fail the continuity test.
#define VERIFY3(SCENTER)                                                     \
    ...
    {                                                                        \
        ...
        for (int cnd = -1; cnd <= 1; cnd++) {                                \
            ...
            float nh = splat_hits(sc, du, p, dir, f0, half_bin, pxw);        \
            if (nh >= 2.0) {                                                 \
                float dt = du * edge_taper(sc, f0);                          \
                if (dt >= win_d) { win_d = dt; win_x = sc; }                 \
            } else if (nh >= 1.0) {                                          \
                float dl = field_exact(sc - pxw, src_y);                     \
                float dr = field_exact(sc + pxw, src_y);                     \
                if (abs(dl - du) < 0.1 && abs(dr - du) < 0.1) {              \
                    float dt = du * edge_taper(sc, f0);                      \
                    if (dt >= w1_d) { w1_d = dt; w1_x = sc; }                \
                }                                                            \
            }                                                                \
        }                                                                    \
        if (win_d < 0.0 && w1_d >= 0.0) { win_d = w1_d; win_x = w1_x; }      \
    }
...
{                                                                            \
    ...
    if (abs(sk - dir * qk * f0 - p) < 3.0 * pxw) {                           \
        ...
        if (win_d >= 0.0) {                                                  \
            ...
            return float4(src_color(win_x, src_y), 1.0);                     \
        }                                                                    \
    }                                                                        \
}
...
float4 mainImage(VertData v_in) : TARGET
{
    ...
    if (debug_mode > 2.5) {
        // Inter-filter transform probes: 3 = raw received rgb at identity,
        // 4 = raw received alpha — settles empirically what OBS does to
        // the bake's output before this filter samples it.
        float xq = (floor(uv.x / uv_pixel_interval.x) + 0.5)
                   * uv_pixel_interval.x;
        float yq = (floor(uv.y / uv_pixel_interval.y) + 0.5)
                   * uv_pixel_interval.y;
        float4 t = image.Sample(textureSampler, safe_uv(xq, yq));
        if (debug_mode > 3.5)
            return float4(t.a, t.a, t.a, 1.0);
    ...
    if (f0 < 0.0005)
        return float4(src_color(p, src_y), 1.0);
    ...
    if (dmax < 0.02) {
        ...
        return float4(src_color(bc, src_y), 1.0);
    }
    ...
    // this bin, it wins outright — skips the march for body interiors.
    {
        ...
        if (du_bc * edge_taper(bc, f0) >= dmax - 0.005 &&
            splat_hits(bc, du_bc, p, dir, f0, half_bin, pxw) >= 2.0) {
